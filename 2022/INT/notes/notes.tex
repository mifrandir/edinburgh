...
\begin{definition}
	...
	\begin{align*}
		G = \{g^k : k \in \N\}.
	\end{align*}
	If such a $g$ exists, we say $G$ is cyclic and write $G=\angles{g}$.
\end{definition}
...
\begin{theorem}[Lecture 8, Theorem 2]
	Let $G=\angles{g}$ and let $\alpha\in\Z$. Then $G=\angles{g^\alpha}$ iff
	$\gcd(\alpha,\abs{G})=1$.
\end{theorem}
...
\begin{theorem}[Lecture 8, Theorem 3]
	...
	$\Z_p^*=\angles{\overline i}$ where $\overline i$ denotes the coset of $i$ in
	$\Z_p$.
\end{theorem}
