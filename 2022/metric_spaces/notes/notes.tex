...
\begin{definition}[Inner product space]
	...
	is a map $\angles{.,.}:X\times X\to\R$ where for all $x,y,z\in X$ and
	$\lambda,\mu\in\R$,
	\begin{enumerate}
		\item $\angles{x,x}\geq 0$ with $\angles{x,x}=0$ iff $x=0$,
		\item $\angles{x,y}=\angles{y,x}$,
		\item $\angles{\lambda x+\mu y,z}=\lambda\angles{x,z}+\mu{y,z}$.
	\end{enumerate}
	A \emph{inner product space} is a pair $(X, \angles{.,.})$ where $X$ is a real
	vector space and $\angles{.,.}$ is a norm on $X$.
\end{definition}

\begin{lemma}
	Let $(X,\angles{.,.})$ be an inner product space. Let $\vabs{-}:X\to\R$ be given
	by
	\begin{align*}
		\vabs x = \sqrt{\angles{x,x}}.
	\end{align*}
...
\begin{proposition}[10.4]
	Let $I\subseteq\R$ an interval. Define $d:I\times I\to\R$ by $\angles{x,y}\mapsto\abs{x-y}$.
