...
loop:   srl  $t1, $s0, 28  # get leftmost digit by shifting it
        ...
        # if $v1 has been set, don't check for leading zeros
        bnez $v1, conv
        # if $t1 is zero, don't bother printing it
        beqz $t1, end
        # if $t1 is not zero, set $v1
        li   $v1, 1 
        # The following instructions convert the number to a char
conv:   slti $t2, $t1, 10  # t2 is set to 1 if $t1 < 10    
        beq  $t2, $0,  over10
...
end:    sll  $s0, $s0, 4   # Drop current leftmost digit
