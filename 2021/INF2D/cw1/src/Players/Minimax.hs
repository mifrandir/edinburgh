...
generateGameTree g = StateTree g $ dropInvalid partialChildren
  where
    dropInvalid :: [(Action, Maybe GameTree)] -> [(Action, GameTree)]
    dropInvalid = map (second fromJust) . filter (isJust . snd)
    partialChildren :: [(Action, Maybe GameTree)]
    partialChildren = [(a, generateGameTree <$> performAction g a) | a <- validActions g]
