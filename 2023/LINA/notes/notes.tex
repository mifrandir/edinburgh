...
\begin{definition}[Orthonormal basis, separable]\label{def:orthonormal-basis}
...
\begin{definition}[Dual space, bounded linear functional]
...
\begin{lemma}[Characterisation of the norm on inner-product spaces]
  Let $X$ be an inner-product space. For all $x\in X$, $\vabs{x}=\sup_{x\in X,\vabs{y}\leq 1} \abs{\aa{x,y}}$.
\end{lemma}

\begin{lemma}[Characterisation of the norm on reflexive spaces]
...
\begin{theorem}[Characterisation of the norm via duals]
  Let $X$ be a normed linear space.
