...
adjacentCitiesNotSameColour paint =
  and
    [ and [not (paint a c && paint b c) | c <- colours]
    | a <- cities
    , b <- cities
    , adj a b
    ]
...
latin n =
  let rows = [0 .. n - 1]
      ...
      noRowHasARepeatedDigit =
        And
          [ Or [N (r, c1, d), N (r, c2, d)]
          | r <- rows
          , c1 <- columns
          , c2 <- drop (c1 + 1) columns
          , d <- digits
          ]
      noColumnHasARepeatedDigit =
        And
          [ Or [N (r1, c, d), N (r2, c, d)]
          | r1 <- rows
          , r2 <- drop (r1 + 1) rows
          , c <- columns
          , d <- digits
...
dpll (And cs) =
  ...
  where
    ...
    prioritise = sortOn (\(Or x) -> length x)
