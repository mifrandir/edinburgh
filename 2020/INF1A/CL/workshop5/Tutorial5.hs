...
reach2 x y = some people (\a -> loves x a && loves a y)

reach3 :: Person -> Person -> Bool
reach3 x y = some people (\a -> reach2 x a && loves a y)
...
check entered
 ...
 =
  ...
  and [or [entered i j k | j <- [1 .. 9]] | i <- [1 .. 9], k <- [1 .. 9]] && -- every column contains every digit
  and [or [entered i j k | i <- [1 .. 9]] | j <- [1 .. 9], k <- [1 .. 9]] && -- every big square contains every digit
  and
    [ or
      [entered (3 * bx + sx) (3 * by + sy) k | sx <- [1 .. 3], sy <- [1 .. 3]]
    | bx <- [0 .. 2]
    , by <- [0 .. 2]
    , k <- [1 .. 9]
    ]
