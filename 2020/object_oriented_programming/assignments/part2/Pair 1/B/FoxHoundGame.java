...
public class FoxHoundGame {
    ...
    private void createPath() {
        path = Paths.get("C:\\Users\\skrlj\\Downloads\\untitled\\src\\game.txt");
    ...
    private static void gameLoop(int dim, String[] players) {
        ...
        FoxHoundGame path1 = new FoxHoundGame();
        path1.createPath();
        ...
        while(!exit) {
            ...
            switch(choice) {
                case FoxHoundUI.MENU_MOVE:
                    while (true) {
                        value = FoxHoundUI.positionQuery(dim, STDIN_SCAN);
                        if (FoxHoundUtils.isValidMove(dim, players, turn, value[0], value[1])) {
                            for (int i = 0; i < players.length; i++) {
                                if ((value[0].charAt(0) == players[i].charAt(0)) && (value[0].charAt(1) == players[i].charAt(1))) {
                                    players[i] = value[1];
                                }
                                }
                            if (FoxHoundUtils.isFoxWin(value[1]))  {
