...
\begin{theorem}
    \B{The Remainder Theorem.} Let $a\in\N$ and $b\in\mathbb{Z}$.
...
\end{theorem}
\begin{definition}
    For two integers $a$ and $b$ we say $a$ \B{divides} $b$, or write $a|b$, if there is an integer $c$ so that $b=ac$.
\end{definition}
\begin{lemma}
    If $a|b$ and $b|a$, then $a=\pm b$.
\end{lemma}
\begin{lemma}
    \B{The Easy Lemma.} Let $a$ and $b$ be integers and suppose $d|a$ and $d|b$. Then $d|ma+nb$ for all $m,n\in\N$.
\end{lemma}
\begin{definition}
    Given two integers $a,b\in\mathbb{Z}$ that are not both zero, the \B{highest common factor} of $a$ and $b$ is the largest positive integer $d\in\N$ that divides both $a$ and $b$.
    We denote this integer $d$ by $\hcf(a,b)$. If $\hcf(a,b)=1$, then we say that $a$ and $b$ are \B{coprime}.
\end{definition}
\begin{theorem}
    \B{Bezout's Identity.} Let $a$ and $b$ be non-zero integers. Then there are integers $s$ and $t$ so that $\hcf(a,b)=sa+tb$.
\end{theorem}
\begin{corollary}
...
\end{corollary}
\begin{theorem}
    \B{The Fundamental Theorem of Arithmetic (FTA).} Let $n\geq 2$ be an integer.
    \begin{itemize}
...
\end{theorem}
\begin{lemma}
    \B{Existence of prime decomposition, Part I.} Every integer $n\geq 2$ can be written as a product of primes $n=p_1\cdots p_k$.
\end{lemma}
\begin{lemma}
    \B{Existence of prime decomposition, Part II.} Every integer $n\geq 2$ can be written as a product of powers of primes $n=p_1^{r_1}\cdots p_k^{r_k}$ and $r_i\geq 0$ for all $i$.
\end{lemma}
\begin{lemma}
    \B{Uniqueness of prime decomposition.} Every integer $n$ can be written as a unique product of powers of primes $n=p_1^{r_1}\cdots p_k^{r_k}$.
\end{lemma}
\begin{theorem}
...
\end{theorem}
\begin{theorem}
    \B{Fermat's Little Theorem (FLT).} If $p$ is prime and $p\not|a\in\Z$, then 
    \begin{align*}
        a^{p-1}\equiv 1 \mod p.
    \end{align*}
\end{theorem}
...
\begin{definition}
    ...
    \begin{enumerate}
        \item We define the \B{union} of $A$ and $B$ to be the set of elements that are either in $A$ or in $B$. We write $A\cup B$.
        \item We define the \B{intersection} of $A$ and $B$ to be the set of elements that are in $A$ and $B$. We write $A\cap B$.
        \item $A$ \B{minus} $B$ is the set of things in $A$ that are not in $B$. We write $A-B$ or $A\setminus B$.
        \item If $A\cap B=\emptyset$, we say $A$ and $B$ are \B{disjoint}.
    \end{enumerate}
\end{definition}
\begin{proposition}
...
\end{proposition}
\begin{definition}
    A \B{partition} of a set $S$ is a collection of nonempty subsets of $S$ so that for every $x\in S$, there is exactly one set $A$ in the collection so that $x\in A$.
\end{definition}
\begin{definition}
    The \B{Cartesian} product of two sets $A$ and $B$ is the set of ordered pairs
    \begin{align*}
        A\times B = \{(x,y) | x\in A,\: y\in B\}.
    \end{align*}
\end{definition}
\begin{definition}
    A \B{relation} $R$ on a set $S$ is just a subset of $S\times S$. Given $a,b\in S$, we write $a\sim b$ if $(a,b)\in R$.
\end{definition}
\begin{definition}
    Let $S$ be a set. Let $\sim$ be a relation on $S$. We say $\sim$ is
    \begin{itemize}
        \item \B{reflexive} if $a\sim a$ for all $a\in S$,
        \item \B{symmetric} if for all $a,b\in S$, we have $a\sim b \Leftrightarrow b\sim a$, and
        \item \B{transitive} if for all $a,b,c\in S$, we have
        \begin{align*}
            a\sim b \sim c \Rightarrow a \sim c.
        \end{align*}
    \end{itemize}
    If $\sim$ satisfies all three of these properties, we say $\sim$ is an \B{equivalence relation}.
\end{definition}
\begin{definition}
    If $\sim$ is an equivalence relation on a set $S$ and $x\in S$, the \B{equivalence class} of $x$ is
    \begin{align*}
        \cl(x) = \{y\in S | x\sim y\}.
    \end{align*}
    The set of \B{equivalence classes} is the set $\{\cl(x) | x \in S\}$.
\end{definition}
\begin{proposition}
...
\section{Functions}
\begin{definition}
    Given two sets $X$ and $Y$, a \B{function from $X$ to $Y$}, denoted $f:X\to Y$, is a relation pairing each $x\in X$ with an element in $Y$ which we denote $f(x)$. The set $X$ is called the \B{domain} of $f$ and $Y$ is the \B{codomain} or \B{range} of $Y$.
\end{definition}
\begin{definition}
    Let $f:X\to Y$ be a function. We say
    \begin{itemize}
        \item $f$ is \B{surjective} if
        \begin{align*}
            \forall y \in Y,\: \exists x \in X \st f(x) = y,
        \end{align*}
        \item $f$ is \B{injective} if
        \begin{align*}
            \forall x,y \in X,\: f(x)=f(y) \Rightarrow x = y,
        \end{align*}
        \item $f$ is \B{bijective} if it is injective and surjective.
    \end{itemize}
\end{definition}
\begin{theorem}
...
\end{theorem}
\begin{definition}
    Let $f:X\to Y$ and $g:Y\to Z$ be functions. We define the \B{composition of $f$ and $g$}, denoted $g\circ f:X\to Z$, to be the function
    \begin{align*}
        g\circ f(x) = g(f(x)).
    \end{align*}
\end{definition}
...
\end{theorem}
\begin{definition}
    If $f:X\to Y$ is bijective, the \B{inverse} of $f$, denoted $f^{-1}:Y\to X$, is the function such that
    \begin{align*}
...
\end{theorem}
\begin{definition}
    If $f:X\to Y$ and $A\subseteq X$, the \B{image of $A$} under $f$ is
    \begin{align*}
        f(A) = \{f(x):x\in A\}\subseteq Y.
    \end{align*}
    If $B\subseteq Y$, the \B{preimage of $B$} under $f$ is
    \begin{align*}
...
\begin{definition}
    We say two sets $A$ and $B$ have the same \B{cardinality} if there is a bijective function $f:A\to B$, and we write $|A|=|B$ or $A\sim B$. If there is an injective map $f:A\to B$, we write $|A|\leq |B|$, and if $|A|\leq |B|$ and there is no injective map from $B$ to $A$, we write $|A|<|B|$.\\
    When $A\sim\N$, we say $A$ is \B{countable}.
\end{definition}
\begin{theorem}
    $\mathbb{Q}\sim\N$.
\end{theorem}
\begin{theorem}
    Let $S$ be an infinite set and let $P(S)$ denote the \B{power set} consisting of the subsets of $S$, i.e. $P(S) = \{A\subseteq S\}$. Then $|S|<|P(S)|$.
\end{theorem}
\section{Counting}
\begin{theorem}
    \B{Multiplication Principle.} Let $P$ be a process consisting of $n$ stages and at each stage $i$, there are $a_i$ many choices we can make, and no two distinct choices can result in the same outcome. 
...
\end{proposition}
\begin{definition}
    Let $S$ be a set of $n$ objects. An \B{ordering} of $S$ is a sequence $a_1,...,a_n$ for which each element of $S$ appears exactly once in this sequence.
\end{definition}
\begin{theorem}
    \B{Rearrangement Theorem.} Given a set $S$ of $n$ distinct objects, there are $n!$ ways of ordering them.
\end{theorem}
\begin{proposition}
...
\begin{definition}
    ...
    \begin{itemize}
        \item A \B{partition} of $S$ is a set $\{A_1,...,A_k\}$ of nonempty subsets of $S$ so that every $x\in\S$ is in exactly one $A_i$.
        \item An \B{ordered partition} of $S$ is a sequence of sets $A_1,...,A_k$ that partition $S$.
    \end{itemize}
\end{definition}
\begin{definition}
    Given nonnegative integers $r_1, ..., r_k$ such that $r_1+\cdots + r_k=n$, we denote the number of \B{ordered} partitions $A_1,..., A_n$ of a set $S$ such that $|A_i=r_i$ as
    \begin{align*}
        \binom{n}{r_1,...,r_k}.
    \end{align*}
\end{definition}
...
\end{theorem}
\begin{theorem}
    \B{The Binomial Theorem.} Let $n\in\N$ and $a,b\in\C$. Then
    \begin{align*}
        (a+b)^n=\sum_{k=0}^n \binom{n}{k}a^kb^{n-k}.
    \end{align*}
\end{theorem}
\begin{theorem}
    \B{The Multinomial Theorem.} Let $n\in\N$ and $x_1, ..., x_k\in\C$. Then $(x_1+\cdots+x_k)^n$ is equal to the sum over all terms of the form
    \begin{align*}
...
\end{theorem}
\begin{theorem}
    \B{Inclusion-Exclusion Principle.} Let $n\geq 2$ and suppose $A_1, A_2, ..., A_n$ are sets. Let
    \begin{align*}
...
\begin{definition}
    ...
    \end{align*}
    We call elements in $S_n$ \B{permutations} of the set $\{1,2,3,4,...,n\}$.
\end{definition}
\begin{lemma}
...
\begin{definition}
    ...
    \begin{align*}
        (a_1\:a_2\:\cdots\:a_r)
    \end{align*}
    and is called a \B{cycle} or \B{cyclic permutation}.
\end{definition}
\begin{corollary}
...
\end{corollary}
\begin{definition}
    If $f=(a_1\:a_2\:\cdots\: a_r)$ and $g=(b_1\:b_2\:\cdots\:b_2)$ where $\{a_1,...,a_r\}$ and $\{b_1,..., b_n\}$ are disjoint sets, we say that $f$ and $g$ are \B{disjoint} in $S_n$.
\end{definition}
\begin{proposition}
...
\begin{definition}
    ...
    \begin{align*}
        f^m = \iota
    \end{align*}
    is called the \B{order} of the permutation $f$.
\end{definition}
\begin{lemma}
...
\end{lemma}
\begin{definition}
    We say a permutation $f\in S_n$ is \B{even} if $\sgn(f)=1$ and \B{odd} if $\sgn(f) = -1$.
\end{definition}
\begin{lemma}
    Let $n\in\N$.
    \begin{enumerate}
