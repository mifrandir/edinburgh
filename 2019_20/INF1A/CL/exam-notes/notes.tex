...
\begin{definition}
    A \B{predicate $p$} defines a subset $\{x\in U\:|\:p\:x\}$ of the universe $U$.
\end{definition}
\begin{definition}
    An \B{Euler Diagram} shows all the subsets defined by all the predicates in a universe and their intersections in such a way that no shown area is empty.
\end{definition}
\begin{definition}
    A \B{Venn Diagram} shows all the subsets defined by all the predicates in a universe and their intersections in such a way that all intersections are shown but they might be empty.
\end{definition}
\begin{theorem}
...
\begin{definition}
    A \B{literal} is a predicate or a negated predicate.
\end{definition}
\begin{definition}
    A \B{disjunctive normal from} is a formula that is a disjunction of conjunctions of literals.\\
    It describes the evaluation that must be true to satisfy the formula.
\end{definition}
\begin{definition}
    A \B{conjunctive normal from} is a formula that is a conjunction of disjunctions of literals.\\
...
\begin{definition}
    Propositions are \B{ordered} by $x\leq y$ iff $x\to y$.
\end{definition}
\begin{theorem}
...
\end{definition}
\begin{definition}
    \B{Regular languages} over an alphabet $\Sigma$:
    \begin{itemize}
...
\end{theorem}
\begin{definition}
    Let $M=(Q,\Sigma,\Delta,B,A)$. Then a \B{trace} for $s=\langle x_0,...,x_{k-1}\rangle\in\Sigma^*$ is a sequence of $k+1$ states such that $\langle q_0, ...,q_k\rangle$ such that all $(q_i,x_i,q_{i+1})\in\Delta$
\end{definition}
\begin{definition}
    An FSM \B{accepts a word} iff there is a trace from some start state $q_0$ to some finish state $q_n$ along transitions that spell out the word.
\end{definition}
\begin{definition}
