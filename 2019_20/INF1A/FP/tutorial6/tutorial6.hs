...
isBloodOrange (Orange s _) = elem s ["Tarocco", "Moro", "Sanguinello"]
isBloodOrange _            = False

-- 2.
bloodOrangeSegments :: [Fruit] -> Int
bloodOrangeSegments fruits = sum [seg fruit | fruit <- fruits]
  where
    seg (Orange _ x) = x
    seg _            = 0
...
-- 3.
worms :: [Fruit] -> Int
worms fruits = length [f | f <- fruits, hasWorm f]
  where
...
tautology wff = and [eval v wff | v <- envs (atoms wff)]
...
equivalent p q =
  and [eval e p == eval e q | e <- envs (nub $ (atoms p) ++ (atoms q))]
...
showWff e = showsPrec 0 e ""
  where
    ...
    showsPrec p (a :<->: b) =
      showParen
        (p > 0)
        (showsPrec 0 a . showSpace . showString "|" . showSpace . showsPrec 0 b)
