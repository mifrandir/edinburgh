...
\begin{itemize}
	\item split the cache into groups of $m$ blocks each (\B{m-way set-associative})
...
\begin{definition}
	\B{Write through} is a strategy where both the memory and the cache
...
\begin{definition}
	\B{Write back} is a strategy where a cache block gets flagged as
...
\begin{definition}
	\B{Write allocate} is a strategy where on a cache miss a block 
...
\begin{definition}
	\B{Write no-allocate} is a strategy on a cache miss the data is
...
\begin{itemize}
	\item each program thinks it owns the entire memory (\B{virtual address space})
	\item actual main memory is \B{physical address space}
...
\begin{definition}
	A cache line or block of VM is called a \B{page}.
	\begin{itemize}
...
\begin{itemize}
	...
	\item Mapping is done through a per-program \B{page table}
\end{itemize}
...
\begin{itemize}
	...
	\item access to a swapped-out page causes a \B{page-fault}
\end{itemize}
Status bits:
\begin{itemize}
...
\begin{definition}
	An \B{exception} is an event that interrupts the normal program flow
...
\begin{itemize}
	...
	\item use the \B{Cause register} to branch to right handler
	\item works well for \T{syscall}
\end{itemize}

Approach 2:
\begin{itemize}
	\item directly jump to a specific handler depending on the exception (\B{vectored interrupt})
...
\begin{itemize}
	...
	\item \B{During the interrupt, not other interrupts should happen!}
\end{itemize}
...
\begin{itemize}
	\item two CPU modes: \B{user} and \B{kernel}
...
States:
\begin{itemize}
	\item \B{RUNNING}: process is currently running
	\item \B{READY}: process is not running, but could run if brought into CPU
	\item \B{BLOCKED}: process is not able to run (i.e. waiting on I/O)
\end{itemize}

Transitions:
\begin{itemize}
	\item \B{I/O REQ}: process initiates I/O
	\item \B{I/O COMP}: I/O finishes
	\item \B{DISPATCH}: Os moves processes into CPU and it starts executing
	\item \B{TIMEOUT}: process's \B{timeslice} is over
\end{itemize}

\begin{definition}
	The \B{PCB} is an OS data structure containing each process's information:
	\begin{itemize}
		\item PID
		\item state
		\item priority
